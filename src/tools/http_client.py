...
def _check_blocked(domain: str) -> bool:
    ...
    try:
        ...
        with engine.connect() as conn:
            row = conn.execute(select(MonitorState).where(MonitorState.domain == domain)).first()
            if row and row[0].blocked_until > datetime.now(UTC):
                logger.warning(
...
def _register_blocked(domain: str) -> None:
    ...
    try:
        ...
        blocked_until = datetime.now(UTC) + timedelta(hours=2)
        engine = create_engine_from_url(settings.database_url)
        with engine.begin() as conn:
            stmt = insert(MonitorState).values(domain=domain, blocked_until=blocked_until)
            stmt = stmt.on_conflict_do_update(
                index_elements=["domain"],
                set_={"blocked_until": blocked_until, "updated_at": datetime.now(UTC)},
            )
            conn.execute(stmt)
...
def fetch(url: str, timeout: int = 30) -> str:
    ...
    if _needs_cloudscraper(domain):
        ...
        try:
            ...
            if hasattr(exc, "response") and getattr(exc.response, "status_code", None) in (
                429,
                403,
            ):
                _register_blocked(domain)
