...
def get_day1_sent_at(session: Any, user_id: str, promo_id: str) -> datetime | None:
    ...
    row = (
        session.query(EmailLog).filter_by(user_id=user_id, promo_id=promo_id, day_number=1).first()
    )
...
def should_send_day(
    ...
    if promo_ends_at and promo_ends_at < now:
        logger.debug("Promoção %s expirada, não envia Dia %d", str(promo_id)[:8], day_number)
        return False
